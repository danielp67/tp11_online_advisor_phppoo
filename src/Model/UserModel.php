...
final class UserModel
{
    ...
    {
        $req = $this->dataBase->prepare('SELECT id, user_login, mail, pass FROM user  WHERE user_login = ? OR mail = ?');
    ...
    public function createNewUser(array $user) :bool
    {
        if ($this->checkUserExist($user['login'], $user['mail']) === false) {
            $newUser = $this->dataBase->prepare('INSERT INTO user (user_login, mail, pass, last_login_at) VALUES(?, ?, ?, ?)');
            return $newUser->execute(array($user['login'], $user['mail'], $user['pass'], $user['lastLoginAt']));
        }
    ...
    {
        $req = $this->dataBase->prepare('SELECT id, user_login, mail, pass, last_login_at FROM user  WHERE user_login = ?');
    ...
    {
        $req = $this->dataBase->prepare('UPDATE user SET last_login_at = ? WHERE user_login = ?');
